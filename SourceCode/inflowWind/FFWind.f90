...
!----------------------------------------------------------------------------------------------------
   ...
   REAL(ReKi), ALLOCATABLE,SAVE          :: FFData  (:,:,:,:)          ! Array of FF data
   REAL(ReKi), ALLOCATABLE,SAVE          :: FFtower (:,:,:)            ! Array of data along the tower, below the FF array

   REAL(ReKi),SAVE                       :: FFDTime                    ! delta time
   REAL(ReKi),SAVE                       :: FFRate                     ! data rate in Hz (1/FFDTime)
   REAL(ReKi),SAVE                       :: FFYHWid                    ! half the grid width
   REAL(ReKi),SAVE                       :: FFZHWid                    ! half the grid height
   REAL(ReKi),SAVE                       :: RefHt                      ! the reference (hub) height of the grid in meters 
   REAL(ReKi),SAVE                       :: GridBase                   ! the height of the bottom of the grid in meters
   REAL(ReKi),SAVE                       :: InitXPosition              ! the initial x location of the wind file (distance the FF file will be offset)
   REAL(ReKi),SAVE                       :: InvFFYD                    ! reciprocal of delta y
   REAL(ReKi),SAVE                       :: InvFFZD                    ! reciprocal of delta z
   REAL(ReKi),SAVE                       :: InvMFFWS                   ! reciprocal of the mean wind speed (MeanFFWS)
   REAL(ReKi),SAVE                       :: MeanFFWS                   ! the mean wind speed (as defined in the FF file), not necessarially the mean of the portion of the wind used
   REAL(ReKi),SAVE                       :: TotalTime                  ! the total time in the simulation

   INTEGER,SAVE                          :: NFFComp                    ! number of wind components
   INTEGER,SAVE                          :: NFFSteps                   ! number of time steps in the FF array
   INTEGER,SAVE                          :: NYGrids                    ! number of points in the lateral (y) direction of the grids
   INTEGER ,SAVE                         :: NZGrids                    ! number of points in the vertical (z) direction of the grids
   INTEGER,SAVE                          :: NTGrids                    ! number of points in the vertical (z) direction on the tower (below the grids)
         
   LOGICAL, SAVE                    :: Initialized = .FALSE.      ! flag that determines if the module has been initialized
   LOGICAL,SAVE                          :: Periodic    = .FALSE.      ! flag that determines if the wind is periodic
...
!----------------------------------------------------------------------------------------------------
   ...
   !-------------------------------------------------------------------------------------------------
   ! Yang: Not wirte this information when coupling with AQWA   
...
TIME_LOOP:  DO IT=1,TmpNumSteps     ! time (add 1 to see if there is an odd number of grids)
   ...
   END DO TIME_LOOP !IT
   ! yang: Not write th when couplin with AQWA
   !IF ( Periodic ) THEN
...
!----------------------------------------------------------------------------------------------------
 ...
 !        ZGOffset = RefHt - GridBase  - FFZHWid
   ...
   !-------------------------------------------------------------------------------------------------
   ! Yang: Not wirte this when coupling with AQWA
   !CALL WrScr1( ' Reading a '//TRIM( Num2LStr(NYGrids) )//'x'//TRIM( Num2LStr(NZGrids) )//  &
   !         ' grid ('//TRIM( Num2LStr(FFYHWid*2) )//' m wide, '// &
   !         TRIM( Num2LStr(GridBase) )//' m to '//TRIM( Num2LStr(GridBase+FFZHWid*2) )//&
