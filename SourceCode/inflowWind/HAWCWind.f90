...
!----------------------------------------------------------------------------------------------------
   ...
   REAL(ReKi), ALLOCATABLE, SAVE          :: WindData  (:,:,:,:)        ! Array of FF data for all 3 wind components

   REAL(ReKi), SAVE                       :: deltaXInv                  ! multiplicative inverse of delta X
   REAL(ReKi), SAVE                       :: deltaYInv                  ! multiplicative inverse of delta Y
   REAL(ReKi), SAVE                       :: deltaZInv                  ! multiplicative inverse of delta Z

   INTEGER, PARAMETER               :: NC = 3                     ! number of wind components
   INTEGER, SAVE                          :: NX                         ! number of points in the X direction
   INTEGER, SAVE                          :: NY                         ! number of points in the Y direction
   INTEGER, SAVE                          :: NZ                         ! number of points in the Z direction

   REAL(ReKi), SAVE                       :: GridBase                   ! the height of the bottom of the grid (Z direction) in meters
   REAL(ReKi), SAVE                       :: LengthX                    ! the grid length in the X direction (distance between point 1 and the next point 1 [because it is periodic])
   REAL(ReKi), SAVE                       :: LengthYHalf                ! half the grid width
   REAL(ReKi), SAVE                       :: RefHt                      ! the reference (hub) height of the grid in meters 
   REAL(ReKi), SAVE                       :: URef                       ! the mean wind speed in m/s at height RefHt meters (as defined in the input file)
