...
!----------------------------------------------------------------------------------------------------
   ...
   REAL(ReKi), ALLOCATABLE,SAVE      :: Tdata  (:)                              ! Time array from the HH wind file
   REAL(ReKi), ALLOCATABLE,SAVE      :: DELTA  (:)                              ! HH Wind direction (angle)
   REAL(ReKi), ALLOCATABLE,SAVE      :: V      (:)                              ! HH horizontal wind speed
   REAL(ReKi), ALLOCATABLE,SAVE      :: VZ     (:)                              ! wind, including tower shadow, along the Z axis
   REAL(ReKi), ALLOCATABLE,SAVE      :: HSHR   (:)                              ! HH Horizontal linear shear
   REAL(ReKi), ALLOCATABLE,SAVE      :: VSHR   (:)                              ! HH vertical shear exponent
   REAL(ReKi), ALLOCATABLE,SAVE      :: VLINSHR(:)                              ! HH vertical linear shear
   REAL(ReKi), ALLOCATABLE,SAVE      :: VGUST  (:)                              ! HH wind gust

   REAL(ReKi),SAVE                   :: LinearizeDels(7)                        ! The delta values for linearization -- perhaps at some point, this could be T/F and we determine the deltas by sqrt(eps) or something similar
   REAL(ReKi),SAVE                   :: RefHt                                   ! reference height; was HH (hub height); used to center the wind
   REAL(ReKi),SAVE                   :: RefWid                                  ! reference width; was 2*R (=rotor diameter); used to scale the linear shear
   
   INTEGER,SAVE                      :: NumDataLines
...
!----------------------------------------------------------------------------------------------------
   ...
   IF ( TimeIndx /= 0 ) THEN  
   ! Yang: not write this when coupling with AQWA   
