...
!----------------------------------------------------------------------------------------------------
   ...
   INTEGER, PARAMETER           :: NumComps  = 3                              ! number of components
            ...
            ! CT_Wind
   REAL(ReKi), SAVE                   :: DelYCTgrid                                 ! The nondimensional distance between grid points in the y direction.
   REAL(ReKi), SAVE                   :: DelZCTgrid                                 ! The nondimensional distance between grid points in the z direction.
   REAL(ReKi), SAVE                   :: CTDistSc                                   ! Disturbance scale (ratio of wave height to rotor diameter).
   REAL(ReKi), SAVE                   :: CTOffset (NumComps)                        ! Offsets to convert integer data to actual wind speeds.
   REAL(ReKi), SAVE                   :: CTScale  (NumComps)                        ! Scaling factors to convert integer data to actual wind speeds.

   
   REAL(ReKi), ALLOCATABLE, SAVE      :: CTvelU   (:,:,:)                         ! The y-z grid velocity data (U components) for the lower- and upper-bound time slices
   REAL(ReKi), ALLOCATABLE, SAVE      :: CTvelV   (:,:,:)                         ! The y-z grid velocity data (V components) for the lower- and upper-bound time slices
   REAL(ReKi), ALLOCATABLE, SAVE      :: CTvelW   (:,:,:)                         ! The y-z grid velocity data (W components) for the lower- and upper-bound time slices
   REAL(ReKi), SAVE                   :: CTLy                                       ! Fractional location of tower centerline from right (looking downwind) to left side of the dataset.
   REAL(ReKi), SAVE                   :: CTLz                                       ! Fractional location of hub height from bottom to top of dataset.
   REAL(ReKi), SAVE                   :: CTScaleVel                                 ! Scaling velocity, U0.  2*U0 is the difference in wind speed between the top and bottom of the wave.
   REAL(ReKi), ALLOCATABLE, SAVE      :: Tdata    (:)                               ! The list of times for the CT-wind input files.  
   
   REAL(ReKi), SAVE                   :: CT_Zref                                    ! The reference height for the CT file (the bottom of the billow)
   REAL(ReKi), SAVE                   :: CTYHWid                                    ! The half the width of the background dataset, used to compute the CTwind time offset
   REAL(ReKi), SAVE                   :: CTYmax                                     ! The dimensional lateral width of the dataset.
   REAL(ReKi), SAVE                   :: CTYt                                       ! Distance of the tower from the right side of the dataset (looking downwind).
   REAL(ReKi), SAVE                   :: CTZmax                                     ! The dimensional vertical height of the dataset.
   REAL(ReKi), SAVE                   :: InvMCTWS                                   ! The multiplicative inverse of the mean hub height wind speed for the CT wind data

   INTEGER, SAVE                      :: CT_DF_Y                                    ! The decimation factor for the CT wind data in the y direction.
   INTEGER, SAVE                      :: CT_DF_Z                                    ! The decimation factor for the CT wind data in the z direction.
   INTEGER, SAVE                      :: CTvel_files(2)                             ! Times for the CT wind files stored in CTvel arrays.

   INTEGER , SAVE                     :: IndCT_hi                                   ! An index into the 3rd dimension of the CTvel arrays, indicating the upper time slice (allows us to avoid copying array)
   INTEGER, SAVE                      :: IndCT_lo                                   ! An index into the 3rd dimension of the CTvel arrays, indicating the lower time slice (allows us to avoid copying array)
   
   INTEGER, SAVE                      :: NumCTt                                     ! The number of CT wind grids, no more than one grid per time step.
   INTEGER, SAVE                      :: NumCTy                                     ! The number of CT wind grid points in the y direction.
   INTEGER , SAVE                     :: NumCTyD                                    ! The decimated number of CT wind grid points in the y direction.
   INTEGER, SAVE                      :: NumCTyD1                                   ! The decimated number of CT wind grid points in the y direction minus 1.
   INTEGER , SAVE                     :: NumCTz                                     ! The number of CT wind grid points in the z direction.
   INTEGER, SAVE                      :: NumCTzD                                    ! The decimated number of CT wind grid points in the z direction.
   INTEGER , SAVE                     :: NumCTzD1                                   ! The decimated number of CT wind grid points in the z direction minus 1.
   INTEGER, SAVE                      :: TimeIndx  = 0                              ! Index into the time array
   INTEGER, ALLOCATABLE , SAVE        :: TimeStpCT (:)                              ! The list of time steps from the original LE simulation, associated with the CT-wind times.

   INTEGER, SAVE                      :: CTWindUnit                                 ! unit number used to read the wind files at each call to CT_GetWindSpeed()
   
   LOGICAL, SAVE                      :: CTVertShft                                 ! Flag to indicate whether or not to shift the z values for the w component.

   CHARACTER(3) , SAVE                :: CText                                      ! The extension used for coherent turbulence data files. (usually "les" or "dns")
   CHARACTER(1024), SAVE              :: CTSpath                                    ! The path to the CT wind files.
