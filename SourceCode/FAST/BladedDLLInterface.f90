...
! Yang: Make the DLL input changable. 10-March-2020
CHARACTER(1024),save              :: DLL_FileName                     ! The name of the DLL file including the full path to the current working directory.
CHARACTER(1024),save              :: DLL_ProcName                         ! The name of the procedure in the DLL that will be called.
CHARACTER(1024),save              :: DLL_InFile                         ! 
! End of the change
...
REAL(4),save                      :: avrSWAP   (R+(2*N)-1)                           ! The swap array, used to pass data to, and receive data from, the DLL controller.
...
INTEGER(4),save                   :: aviFAIL                                         ! A flag used to indicate the success of the DLL call as follows: 0 if the DLL call was successful, >0 if the DLL call was successful but cMessage should be issued as a warning messsage, <0 if the DLL call was unsuccessful or for any other reason the simulation is to be stopped at this point with cMessage as the error message
...
INTEGER(4),save                   :: Sttus                                           ! Status returned by an attempted allocation.

INTEGER(1),save                   :: accINFILE (  256)                               ! The address of the first record of an array of 1-byte CHARACTERs giving the name of the parameter input file, 'DISCON.IN'.
INTEGER(1),save                   :: avcMSG    (  256)                               ! The address of the first record of an array of 1-byte CHARACTERS returning a message that will be displayed if aviFAIL <> 0.
INTEGER(1),save                   :: avcOUTNAME( 1024)                               ! The address of the first record of an array of 1-byte CHARACTERS giving the simulation run name without extension.
INTEGER(1),save                   :: iInFile   (  256)                               ! CHARACTER string cInFile  stored as a 1-byte array.
INTEGER(1),save                   :: iMessage  (  256)                               ! CHARACTER string cMessage stored as a 1-byte array.
INTEGER(1),save                   :: iOutName  ( 1024)                               ! CHARACTER string cOutName stored as a 1-byte array.

CHARACTER( 256),save              :: cInFile                                         ! CHARACTER string giving the name of the parameter input file, 'DISCON.IN'
CHARACTER( 256),save              :: cMessage                                        ! CHARACTER string giving a message that will be displayed by the calling program if aviFAIL <> 0.
